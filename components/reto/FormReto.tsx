...
export default function FormReto({ codUsuarioReto, schema, fetchJson, reto, onSent }: Props) {
    ...
    useEffect(() => {
        setValues(initGenericValues(schema));
    }, [schema]);
    ...
    const send = async () => {
        ...
        try {
            const r = await fetchJson(`/mis-retos/${codUsuarioReto}/form/enviar`, asJson({
                codUsuarioReto,
                codReto: reto.codReto,
                data: { kind: 'genericForm', data: values },
            }));
            ...
            if (t === 'array') {
                ...
                return (
                    <View key={k} style={{ marginTop: 18 }}>
                        ...
                        {(arrVal || []).map((row, idx) => (
                            <View key={idx} style={{ marginTop: 10, padding: 10, borderWidth: 1, borderColor: colors.divider, borderRadius: 10 }}>
                                ...
                                <Pressable
                                    ...
                                    style={{ marginTop: 10, alignSelf: 'flex-start', paddingVertical: 6, paddingHorizontal: 12, borderRadius: 999, backgroundColor: '#fdecea', borderWidth: 1, borderColor: '#f5c6cb' }}
                                >
                        ...
                        <Pressable
                            onPress={addRow}
                            style={{ marginTop: 10, alignSelf: 'flex-start', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 999, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.divider }}
                        >
        ...
        <ScrollView>
            {Object.keys(schema || {}).length === 0 ? (
                ...
                Object.entries(schema).map(([k, def]) => (
                    <View key={k}>{renderField(k, def as AnyField, values?.[k], (newVal) => setValues((p: any) => ({ ...p, [k]: newVal })))}</View>
                ))
            )}
            ...
            <Pressable
                ...
                style={{
                    ...
                    backgroundColor: ok ? '#0d6efd' : '#cfd3da',
                    borderRadius: 999,
                    alignSelf: 'center',
                    opacity: ok ? 1 : 0.75,
                }}
