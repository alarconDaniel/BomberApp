...
// 🔊 Sonidos
import { initSoundEngine, preloadAll, play as playSound } from '../../utils/sound';
...
export default function OperariosScreen() {
  ...
  // 🔔 Detección de altas (IDs nuevos) con campanita
  const prevIdsRef = useRef<Set<string>>(new Set());
  const hasLoadedRef = useRef(false);

  // Inicializar audio y precargar para evitar lags
  useEffect(() => {
    (async () => {
      try {
        ...
        // silencio: si algo falla aquí, no bloquea la UI
      }
    })();
  }, []);
  ...
  // Aplica lista, detecta altas y reproduce "campana" si corresponde (pero no en la primera carga)
  const applyList = useCallback((arr: OperarioUI[]) => {
    const prev = prevIdsRef.current;
    const next = new Set(arr.map(o => o.id));

    let added = false;
    for (const id of next) {
      if (!prev.has(id)) { added = true; break; }
    }

    if (hasLoadedRef.current && added) {
      try { playSound('campana'); } catch {}
    }
  ...
  // ⬇️ Mantengo la estructura de incoming pero usando applyList para el sonido
  const listar = useCallback(async () => {
    try {
      ...
      // incoming: silencio en errores de lista
    } finally {
      setCargando(false);
    }
  }, [fetchJson, applyList]);
  ...
  const borrar = (id: string) => {
    ...
    Alert.alert('Confirmar', '¿Deseas borrar este usuario?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          let exito = false; // ✅ conservar lógica incoming
          try {
            setCargando(true);
            await fetchJson(`/usuario/borrar/${Number(id)}`, { method: 'DELETE' });

            // 🔊 Sonido al eliminar (sólo tras éxito del DELETE)
            try { await playSound('eliminar'); } catch {}
            ...
            setCargando(false);
            // ✅ Alerta de Información (incoming) conservada
            if (exito) {
  ...
  return (
    <FadeWrapper>
      <SafeAreaView style={[s.container, { paddingTop: (insets.top || 8) }]}>
        {/* Header */}
        <View style={s.header}>
          <Text style={[g.text.h1, { textAlign: 'center', alignSelf: 'center', paddingTop: 10 }]}>
            Operarios
          </Text>
...
function getStyles(c: import('../../theme/ThemeProvider').Palette) {
  return StyleSheet.create({
    ...
    header: {
      ...
      paddingBottom: 10,
      marginTop: 3, // incoming: pequeño ajuste visual
    },
    primaryBtn: {
      flexDirection: 'row',
      alignItems: 'center',
