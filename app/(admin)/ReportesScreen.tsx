...
export default function ReportesScreen() {
  ...
  function mapRowToItem(row: any): FileItem {
    ...
    return { key, size: sizeNum, lastModified: when || undefined, area };
  }

  // ---- Cargar lista desde /uploads/list (BD)
  const loadFiles = useCallback(async () => {
    const take = 100, skip = 0;
    const url = API.uploads.list(codUsuario, take, skip);
    const r = await fetchJson<{ total: number; rows: any[] }>(url);
  ...
  const onRefresh = useCallback(async () => {
    try { setRefreshing(true); await cargar(); }
  ...
  // ---- Descargar (signed GET)
  const onDownload = useCallback(async (key: string) => {
    try {
      const { url } = await fetchJson<{ url: string; expiresIn: number }>(
        API.uploads.downloadPresign(key)
      );
  ...
  const onDelete = useCallback((key: string) => {
    Alert.alert('Eliminar', `¿Borrar este archivo?\n${key}`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const del = API.uploads.delete(key, codUsuario);
            await fetchJson(del.url, del.init);
  ...
  const secciones = useMemo(() => {
    ...
    const base = q ? files.filter(f => (f.key || '').toLowerCase().includes(q)) : files;
  ...
  return (
    <FadeWrapper>
      ...
      <View style={[styles.container, { backgroundColor: c.bg }]}>
        ...
        {/* Selector de tipo para la subida */}
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 10 }}>
          ...
          >
            ...
            <Text style={{ color: '#fff', fontWeight: '700', marginLeft: 8 }}>Subir archivo{selectedTipo ? ` (${selectedTipo})` : ''}</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: c.pill }]}
            onPress={onRefresh}
            activeOpacity={0.9}
          >
            <Ionicons name="refresh" size={18} color={c.text} />
            <Text style={{ color: c.text, fontWeight: '700', marginLeft: 8 }}>Refrescar</Text>
          </TouchableOpacity>
        </View>
