...
export default function RetosScreen() {
  ...
  /* ======== EDITAR: rellena form y baja ======== */
  const editar = (item: RetoDTO) => {
    setEditingId(item.codReto);
    setNombre(item.nombreReto || '');
    setDescripcion(item.descripcionReto || '');
    setTipo(null);
    setShowForm(true);
    requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: formY, animated: true }));
  };
  const cancelarEdicion = () => {
    setEditingId(null); setNombre(''); setDescripcion(''); setTipo(null); setShowForm(false);
  };
  ...
  const crearReto = async () => {
    if (!isAdmin) { Alert.alert('Sin permisos', 'Solo un administrador puede crear retos.'); return; }
  ...
  return (
    <FadeWrapper>
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
        ...
        >
          {/* Header */}
          <View style={{ paddingHorizontal: 18, paddingTop: insets.top + 8, paddingBottom: 6 }}>
            <Text style={g.text.h2}>Retos</Text>
          </View>
          ...
          {/* Formulario */}
          <View onLayout={e => setFormY(e.nativeEvent.layout.y)} style={[s.card, { marginTop: 16 }]}>
            <Text style={g.text.h3}>{editingId ? 'Editar reto' : 'Crear un nuevo reto'}</Text>

            {editingId && (
              <View style={[s.panel, { borderStyle: 'dashed' }]}>
                <Text style={[g.text.smallStrong]}>Editando ID: {editingId}</Text>
                <Text style={[g.text.caption, { marginTop: 4 }]}>
                  Se rellenó el formulario con los datos del reto seleccionado.
                </Text>
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                  <TouchableOpacity style={[s.secondaryBtn, { borderColor: colors.outline }]} onPress={cancelarEdicion}>
                    <Text style={g.text.bodyStrong}>Cancelar edición</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
            ...
            {showForm ? (
              <>
                <Text style={[g.text.smallStrong, { marginTop: 6, marginBottom: 6 }]}>Nombre del tema</Text>
                <TextInput
                  value={nombre}
                  ...
                  <View style={s.panel}>
                    ...
                    {opciones.map((o, idx) => (
                      <View key={o.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <Pressable
                          onPress={() => {
                            setOpciones(prev => prev.map(p => p.id === o.id ? { ...p, correcta: !p.correcta } : p));
                          }}
              ...
              <TouchableOpacity
                style={[s.primaryBtn, { backgroundColor: colors.primarySoft, marginTop: 8 }]}
                onPress={() => setShowForm(true)}
              >
                <Text style={g.text.bodyStrong}>Abrir formulario</Text>
              </TouchableOpacity>
            )}
          </View>
        </ScrollView>
...
function getStyles(c: import('../../theme/ThemeProvider').Palette) {
  return StyleSheet.create({
    ...
    secondaryBtn: {
      ...
      paddingHorizontal: 14, backgroundColor: c.mutedBg, borderWidth: 1,
    },
    searchWrap: {
