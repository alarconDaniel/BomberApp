...
// 🔔 Snapshot persistente de IDs para detectar altas (creaciones)
let LAST_SEEN_IDS: Set<string> | null = null;
...
export default function RetosScreen() {
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      // 🔔 Detectar creación: si aparecen IDs nuevos vs el último snapshot, sonar campana
      const nextIds = new Set(arr.map(r => String(r.codReto)));
      let added = false;
      if (LAST_SEEN_IDS) {
        for (const id of nextIds) {
          if (!LAST_SEEN_IDS.has(id)) { added = true; break; }
        }
      }
      if (LAST_SEEN_IDS && added) {
        try { playSound('campana'); } catch {}
      }
      LAST_SEEN_IDS = nextIds;
    ...
    Alert.alert('Confirmar', `¿Borrar el reto "${item.nombreReto}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            setLoading(true);
            // Soporta firmas con o sin nombre (según lib)
            await deleteReto(fetchJson, item.codReto, (item as any)?.nombreReto);
            exito = true;

            // 🔊 Sonar eliminar tras éxito del borrado
            try { playSound('eliminar'); } catch {}
